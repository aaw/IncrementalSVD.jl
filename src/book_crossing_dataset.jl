...
function load_book_crossing_dataset(;
    ...
    isbn_to_title = Dict{AbstractString,AbstractString}()
    ...
    user_distinct_ratings_set = Dict{AbstractString,Set{AbstractString}}()
    book_distinct_ratings_set = Dict{AbstractString,Set{AbstractString}}()
    ...
    for line in eachline(ratings_file)
        ...
        if !haskey(user_distinct_ratings_set, user)
            user_distinct_ratings_set[user] = Set{AbstractString}()
        end
        push!(user_distinct_ratings_set[user], rating)
        if !haskey(book_distinct_ratings_set, isbn)
            book_distinct_ratings_set[isbn] = Set{AbstractString}()
        end
